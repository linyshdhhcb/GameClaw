package ai.gameclaw.tools.search;
...
class DynamicToolDiscoveryConfigurationTest {
    ...
    void whenEnabled_registersToolSearcherAndAdvisor() {
        contextRunner
                .withPropertyValues(
                        "gameclaw.tools.dynamic-discovery.enabled=true",
                        "gameclaw.tools.dynamic-discovery.max-results=7",
                        "gameclaw.tools.dynamic-discovery.lucene-min-score-threshold=0.0"
                )
                .run(context -> {
                    assertThat(context).hasSingleBean(ToolSearcher.class);
                    assertThat(context).hasSingleBean(ToolSearchToolCallAdvisor.class);
    ...
    void whenDisabled_doesNotRegisterToolSearcherOrAdvisor() {
        contextRunner
                .withPropertyValues("gameclaw.tools.dynamic-discovery.enabled=false")
                .run(context -> {
